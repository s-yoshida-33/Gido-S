...
// Default Genre Global Settings
const DEFAULT_GENRE_GLOBAL_SETTINGS = {
  maxItems: 3,
  ignoredKeywords: [
    "WAONPOINT加盟店",
    "AEONPayが使えるお店",
...
// Load bundled default settings lazily
let _bundledDefaults = null;

function getBundledDefaults() {
  if (_bundledDefaults !== null) return _bundledDefaults;

  _bundledDefaults = {};

  try {
    // Try to find default-settings.json in likely locations
    const possiblePaths = [
      path.join(__dirname, 'default-settings.json'),
      // In asar, __dirname is .../app.asar/electron
      // app.getAppPath() is .../app.asar
      path.join(app.getAppPath(), 'electron', 'default-settings.json'),
    ];

    // Add dev path if not packaged
    if (!app.isPackaged) {
      possiblePaths.push(path.join(process.cwd(), 'electron', 'default-settings.json'));
    }
    ...
    if (defaultsPath) {
      logger.info(`Loading bundled default settings from: ${defaultsPath}`);
      const raw = fs.readFileSync(defaultsPath, 'utf-8');
      _bundledDefaults = JSON.parse(raw);
      logger.info('Successfully loaded bundled default settings');
    } else {
      logger.warn('default-settings.json not found in expected locations', { possiblePaths });
    }
  } catch (e) {
    logger.error('Failed to load bundled default settings', { error: e.message });
  }
...
function loadSettings() {
  ...
  const initialLocationIcons = defaults.locationIcons || DEFAULT_LOCATION_ICON_SETTINGS;
  const initialFloorLayout = defaults.floorLayout || DEFAULT_FLOOR_LAYOUT;
  const initialGenreMappings = defaults.genreMappings || DEFAULT_GENRE_MAPPINGS;
  const initialGenreGlobalSettings = defaults.genreGlobalSettings || DEFAULT_GENRE_GLOBAL_SETTINGS;

  const base = {
    floor: '1F',
    locationIcons: initialLocationIcons,
    floorLayout: initialFloorLayout,
    genreMappings: initialGenreMappings,
    genreGlobalSettings: initialGenreGlobalSettings,
    imageSettings: {
      floorMaps: {
        '1F': '',
        '2F': '',
  ...
  try {
    ...
    const merged = {
      ...
      genreMappings: (() => {
        ...
        // If we have stored settings, use them as the source of truth for keys and order
        if (stored && typeof stored === 'object' && Object.keys(stored).length > 0) {
          const result = {};
          // Preserve order from stored settings
          Object.keys(stored).forEach(key => {
            const val = stored[key];
            const defaultVal = DEFAULT_GENRE_MAPPINGS[key] || {};
            
            if (typeof val === 'string') {
              // Migration: convert string (labelEn only) to object with defaults
              result[key] = {
                ...DEFAULT_GENRE_CONFIG,
                ...defaultVal, // Apply default colors etc if available
                labelEn: val, // Override label with stored value
              };
              ...
              result[key] = {
                ...DEFAULT_GENRE_CONFIG,
                ...defaultVal, // Apply default colors etc if available
                ...val, // Override with user settings
              };
            }
          });
          return result;
        ...
        // Fallback to defaults if no stored settings
        return { ...base.genreMappings };
      })(),
      genreGlobalSettings: parsed.genreGlobalSettings || base.genreGlobalSettings,
      locationIcons: {
...
/**
 * Broadcast genre global settings changes to renderer processes
 */
function broadcastGenreGlobalSettings(settings) {
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('genre-global-settings-updated', settings);
  }
}
...
function updateFloorLayout(floor, partialLayout) {
  ...
  const prevLayout = current.floorLayout || initialFloorLayout;
  const prevForFloor = prevLayout[floor] || initialFloorLayout[floor] || {};
...
function createMainWindow() {
  ...
  mainWindow.webContents.on('did-finish-load', () => {
    ...
    broadcastGenreGlobalSettings(settings.genreGlobalSettings);
...
ipcMain.handle('settings:get-floor-layout', () => {
  ...
  const defaults = getBundledDefaults();
  const initialFloorLayout = defaults.floorLayout || DEFAULT_FLOOR_LAYOUT;
  return settings.floorLayout || initialFloorLayout;
});
...
ipcMain.handle('get-genre-mappings', () => {
  ...
  const defaults = getBundledDefaults();
  const initialGenreMappings = defaults.genreMappings || DEFAULT_GENRE_MAPPINGS;
  return settings.genreMappings || initialGenreMappings;
});
...
ipcMain.handle('get-genre-global-settings', () => {
  const settings = loadSettings();
  logger.debug('IPC get-genre-global-settings');
  const defaults = getBundledDefaults();
  return settings.genreGlobalSettings || defaults.genreGlobalSettings || DEFAULT_GENRE_GLOBAL_SETTINGS;
});

ipcMain.handle('save-genre-global-settings', (_event, genreGlobalSettings) => {
  logger.info('IPC save-genre-global-settings');
  const settings = saveSettings({ genreGlobalSettings });
  broadcastGenreGlobalSettings(settings.genreGlobalSettings);
  return settings.genreGlobalSettings;
});
